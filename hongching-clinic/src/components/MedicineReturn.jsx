...
export default function MedicineReturn({ data, showToast, user }) {
  ...
  const handlePrint = (r) => {
    const rows = (r.items || []).map((it, i) => `<tr><td style="text-align:center">${i + 1}</td><td>${it.medicine}</td><td style="text-align:center">${it.qty}</td><td style="text-align:right">$${it.unitPrice}</td><td style="text-align:right">$${(it.qty || 0) * (it.unitPrice || 0)}</td><td>${it.reason}</td></tr>`).join('');
    const st = STATUS_MAP[r.status] || STATUS_MAP.pending;
    const w = window.open('', '_blank');
    if (!w) return;
    w.document.write(`<!DOCTYPE html><html><head><title>退藥單 - ${r.patientName}</title><style>@page{size:A4;margin:15mm}body{font-family:'PingFang TC','Microsoft YaHei',sans-serif;font-size:12px;padding:24px;max-width:700px;margin:0 auto}h1{font-size:17px;text-align:center;margin:0 0 4px}p.sub{text-align:center;color:#666;font-size:11px;margin:0 0 16px}table{width:100%;border-collapse:collapse;margin:12px 0}th,td{padding:6px 10px;border:1px solid #ddd;font-size:12px}th{background:#f3f4f6;font-weight:700}.row{display:flex;justify-content:space-between;margin:4px 0;font-size:12px}.total{font-size:15px;font-weight:700;text-align:right;margin:12px 0;color:#0e7490}.sign{display:flex;justify-content:space-between;margin-top:40px;font-size:12px}.sign div{border-top:1px solid #333;padding-top:4px;width:140px;text-align:center}@media print{body{padding:10px}}</style></head><body><h1>${clinicName}</h1><p class="sub">退藥 / 退款收據</p><div class="row"><span>退藥單號：${r.id}</span><span>狀態：${st.label}</span></div><div class="row"><span>病人：${r.patientName}</span><span>日期：${r.returnDate}</span></div><table><thead><tr><th>#</th><th>藥物名稱</th><th>數量</th><th>單價</th><th>小計</th><th>原因</th></tr></thead><tbody>${rows}</tbody></table><div class="total">退款總額：$${r.totalRefund || 0}</div><div class="row"><span>退款方式：${r.refundMethod || '-'}</span><span>退款日期：${r.refundDate || '-'}</span></div>${r.notes ? `<div style="margin:12px 0;font-size:11px;color:#555">備註：${r.notes}</div>` : ''}<div class="sign"><div>病人簽署</div><div>經手人：${r.processedBy || r.createdBy || '-'}</div></div></body></html>`);
    w.document.close();
