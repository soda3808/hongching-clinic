...
export default function MedicalSupplyOrder({ data, showToast, user }) {
  ...
  const printOrder = (order) => {
    ...
    const rows = order.items.map(i => `<tr><td style="border:1px solid #ccc;padding:6px">${i.name}</td><td style="border:1px solid #ccc;padding:6px;text-align:center">${i.qty}</td><td style="border:1px solid #ccc;padding:6px;text-align:right">$${Number(i.unitPrice).toFixed(2)}</td><td style="border:1px solid #ccc;padding:6px;text-align:right">$${(Number(i.qty) * Number(i.unitPrice)).toFixed(2)}</td></tr>`).join('');
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>採購單</title><style>body{font-family:Arial,sans-serif;padding:30px}table{width:100%;border-collapse:collapse;margin-top:16px}th{background:${ACC};color:#fff;padding:8px;text-align:left}h1{color:${ACC}}.header{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid ${ACC};padding-bottom:12px;margin-bottom:20px}.meta{margin-bottom:16px;line-height:1.8}.total{text-align:right;font-size:18px;font-weight:bold;margin-top:12px;color:${ACC}}.footer{margin-top:40px;display:flex;justify-content:space-between}.sig{border-top:1px solid #333;width:200px;text-align:center;padding-top:6px}@media print{body{padding:10px}}</style></head><body><div class="header"><div><h1>${clinicName}</h1><div>醫療物資採購單</div></div><div style="text-align:right"><div>單號：${order.orderNo}</div><div>日期：${(order.createdAt || '').substring(0, 10)}</div></div></div><div class="meta"><div>供應商：${order.supplier}</div><div>狀態：${STATUS_MAP[order.status]?.label}</div>${order.notes ? `<div>備註：${order.notes}</div>` : ''}</div><table><thead><tr><th>品名</th><th style="text-align:center">數量</th><th style="text-align:right">單價</th><th style="text-align:right">小計</th></tr></thead><tbody>${rows}</tbody></table><div class="total">總計：$${order.total.toFixed(2)}</div><div class="footer"><div class="sig">採購人簽名</div><div class="sig">主管批核</div><div class="sig">供應商確認</div></div><script>setTimeout(()=>window.print(),300)<\/script></body></html>`);
    w.document.close();
  };
