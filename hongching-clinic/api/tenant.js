// Tenant API — update tenant configuration (self-service)
// POST /api/tenant  (was /api/tenant/update)
...
export default async function handler(req, res) {
  ...
  if (!['admin', 'manager', 'superadmin'].includes(user.role)) return res.status(403).json({ error: 'Insufficient permissions' });
  ...
  try {
    ...
    await supabase.from('audit_logs').insert({ tenant_id: tenantId, user_id: user.userId, action: 'update', entity: 'tenant', entity_id: tenantId, details: { fields: Object.keys(updates), updatedBy: user.name } }).catch(() => {});

    return res.status(200).json({
      success: true,
      tenant: { id: data.id, slug: data.slug, name: data.name, nameEn: data.name_en, logoUrl: data.logo_url, stores: data.stores || [], doctors: data.doctors || [], services: data.services || [], settings: data.settings || {} },
    });
